...
def hinge_loss(predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    ...
    losses = torch.empty([batch_size, input_size], dtype=torch.float32, device=predictions.device)
...
def check(batch_size: int, input_shape: tuple[int, ...]) -> None:
    ...
    """
    predictions = torch.randn([batch_size, *input_shape], device=DEVICE, dtype=torch.float32)
