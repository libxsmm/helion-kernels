...
def pytorch_baseline(anchor: torch.Tensor, positive: torch.Tensor, negative: torch.Tensor) -> torch.Tensor:
...
def check(batch_size: int, features: int) -> None:
    ...
    anchor = torch.rand(batch_size, features, device=DEVICE, dtype=torch.float16) * scale
