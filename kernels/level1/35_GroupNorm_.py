...
def groupnorm_kernel(
    ...
    for tile_b, tile_c in hl.tile([batch_size, num_features], block_size=[None, channels_per_group]):        
        # Extract group data using slicing
        group_data = x_reshaped[tile_b, tile_c, :].to(torch.float32)  # Shape: [tile_b, channels_per_group, H*W]
        
        # Compute mean by reducing one dimension at a time
        # First reduce over height and width
        mean_w = torch.mean(group_data, dim=-1, keepdim=True)  # [tile_b, channels_per_group, 1]
        ...
        # First reduce over height and width
        var_w = torch.mean(squared_diff, dim=-1, keepdim=True)  # [tile_b, channels_per_group, 1]
        # Then reduce over channels
...
class Model:
    ...
    def __init__(self, num_features: int, num_groups: int, dtype: torch.dtype = None):
        ...
        """
        self.gn = nn.GroupNorm(num_groups=num_groups, num_channels=num_features, dtype=dtype)
...
def check(batch_size: int, features: int, num_groups: int, dim1: int, dim2: int) -> None:
    """
    ...
    """
    x = torch.randn([batch_size, features, dim1, dim2], device=DEVICE, dtype=torch.float32)
    
    # Create a GroupNorm layer and extract its parameters
    gn = nn.GroupNorm(num_groups=num_groups, num_channels=features, dtype=x.dtype).to(DEVICE)
