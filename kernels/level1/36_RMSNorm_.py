...
def rmsnorm_kernel(
    ...
    for tile_b, tile_h, tile_w in hl.tile([batch_size, height, width]):
        ...
        spatial_slice = x[tile_b, :, tile_h, tile_w].to(torch.float32)  # Shape: [tile_b, num_features, tile_h, tile_w]
        
        # Compute mean of squared values across feature dimension
        mean_squared = torch.mean(spatial_slice * spatial_slice, dim=-3, keepdim=True)  # [tile_b, 1, tile_h, tile_w]
...
def check(batch_size: int, features: int, dim1: int, dim2: int) -> None:
    ...
    # Test RMSNorm
    run_example(
        lambda x: rmsnorm_kernel(x, eps=1e-5),
        pytorch_baseline,
        (x,)
    )
