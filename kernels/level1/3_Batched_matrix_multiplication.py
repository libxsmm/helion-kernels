...
def batched_matmul(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    assert batch_size == batch_size2 and k == k2, f"size mismatch: A{A.size()}, B{B.size()}"
    
    out = torch.empty([batch_size, m, n], dtype=torch.promote_types(A.dtype, B.dtype), device=A.device)
