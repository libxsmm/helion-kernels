...
def hardsigmoid_kernel(x: torch.Tensor) -> torch.Tensor:
    """
    Applies HardSigmoid activation to the input tensor using Helion.
    ...
    Returns:
        Output tensor with HardSigmoid applied, same shape as input
    """
    ...
    for tile_idx in hl.tile(total_elements):
        ...
        output_slice = torch.nn.functional.hardsigmoid(input_slice)
...
def check(batch_size: int, dim: int) -> None:
    """
    Checks the correctness of the HardSigmoid kernel against PyTorch baseline.
    ...
    run_example(hardsigmoid_kernel, pytorch_baseline, (x,))
