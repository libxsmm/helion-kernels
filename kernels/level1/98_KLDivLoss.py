...
def kl_div_kernel(
    ...
    for tile_b in hl.tile(batch_size):
        for tile_f in hl.tile(features):
            ...
            kl_contribution = target_clamped * (torch.log(target_clamped) - torch.log(pred_clamped))
...
def check(batch_size: int, features: int) -> None:
    ...
    predictions = (torch.rand(batch_size, features, device=DEVICE, dtype=torch.float16) * scale).softmax(dim=-1)
    targets = torch.rand(batch_size, features, device=DEVICE, dtype=torch.float16).softmax(dim=-1)
