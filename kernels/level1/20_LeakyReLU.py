...
def leaky_relu_kernel(x: torch.Tensor, negative_slope: float = 0.01) -> torch.Tensor:
    ...
    for tile_idx in hl.tile(total_elements):
        ...
        output_slice = torch.nn.functional.leaky_relu(
            input_slice, negative_slope=negative_slope
        )
...
def check(batch_size: int, dim: int) -> None:
    ...
    run_example(
        lambda x: leaky_relu_kernel(x, negative_slope=0.01), pytorch_baseline, (x,)
    )
