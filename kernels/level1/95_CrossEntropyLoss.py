...
def cross_entropy_loss_kernel(
    ...
    for tile_b in hl.tile(batch_size):  # [tile_b]
        ...
        for tile_c in hl.tile(num_classes):  # [tile_c]
            ...
            target_mask = (
                target_indices[:, None] == class_indices[None, :]
            )  # [tile_b, tile_c]

            # Extract target logits
            masked_logits = torch.where(
                target_mask, pred_slice, 0.0
            )  # [tile_b, tile_c]
...
def check(batch_size: int, num_classes: int) -> None:
    predictions = torch.rand(
        batch_size, num_classes, device=DEVICE, dtype=torch.float16
    )
    targets = torch.randint(0, num_classes, (batch_size,), device=DEVICE)

    run_example(cross_entropy_loss_kernel, pytorch_baseline, (predictions, targets))
