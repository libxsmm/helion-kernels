...
def instancenorm_kernel(
    ...
    for tile_b, tile_c in hl.tile([batch_size, num_features]):
        ...
        spatial_flat = x_reshaped[tile_b, tile_c, :].to(torch.float32)  # Shape: [tile_b, tile_c, H*W]
...
def instancenorm_kernel_multi_stage(
    ...
    for tile_b, tile_c in hl.tile([batch_size, num_features]):
        ...
        for tile_h, tile_w in hl.tile([height, width]):
            ...
            normalized = (spatial_slice - instance_mean[:, :, None, None]) / torch.sqrt(instance_var[:, :, None, None] + eps)
