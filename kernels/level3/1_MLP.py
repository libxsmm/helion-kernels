...
def mlp_kernel(
    x: torch.Tensor,
    weight1: torch.Tensor, bias1: torch.Tensor,
    weight2: torch.Tensor, bias2: torch.Tensor,
    weight3: torch.Tensor, bias3: torch.Tensor,
) -> torch.Tensor:
    """
...
class Model(nn.Module):
    def __init__(self, input_size, layer_sizes, output_size):
        ...
        for layer_size in layer_sizes:
            self.layers.append(nn.Linear(current_input_size, layer_size, dtype=torch.float16))
            self.layers.append(nn.ReLU())
            current_input_size = layer_size
        
        self.layers.append(nn.Linear(current_input_size, output_size, dtype=torch.float16))
...
def check(batch_size: int, input_size: int, layer_sizes: list, output_size: int) -> None:
    ...
    run_example(
        lambda x: mlp_kernel(x, layer1.weight, layer1.bias, 
